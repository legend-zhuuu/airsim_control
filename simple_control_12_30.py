...
class SimpleTerminalController:
    ...
    def drone_move(self, args: list):
        print("MoveOnPath received")
        self.client.moveToPositionAsync(x=0, y=0, z=-200, velocity=10, timeout_sec=30, vehicle_name="Drone1").join()
        self.client.moveByVelocityAsync(vx=3, vy=0, vz=0, duration=20, vehicle_name="Drone1").join()
        self.client.moveByVelocityAsync(vx=0, vy=3, vz=0, duration=20, vehicle_name="Drone1").join()
        self.client.moveByVelocityAsync(vx=-3, vy=0, vz=0, duration=20, vehicle_name="Drone1").join()
        self.client.moveByVelocityAsync(vx=0, vy=3, vz=0, duration=20, vehicle_name="Drone1").join()
    ...
    def saveimg(self):
        ...
        responses = self.client.simGetImages([
            ...
            airsim.ImageRequest("3", airsim.ImageType.Scene),  #scene vision image in uncompressed RGBA array
            airsim.ImageRequest("3",airsim.ImageType.Segmentation)  #segmentation vision image
            ])
        ...
        for idx, response in enumerate(responses):
            if idx == 0:
                ...
                filename = os.path.join(out_dir, str(3) + "_" + str(idx) + "_" + str(timestamp))
            elif idx == 1:
